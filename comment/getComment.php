    ...
    try {
        ...
        if ($chapterId <= 0) {
            ...
            $stmt = $conn->prepare("
                ...
                WHERE comment.chapter_id = ?
                ORDER BY comment.time DESC
                LIMIT ?,4
            ");
            $stmt->bind_param("ii", $chapterId, $offset);
            $stmt->execute();
            ...
            $stmt->close();
             ...
             // 获取评论总数
            $totalStmt = $conn->prepare("SELECT COUNT(*) as total FROM comment WHERE chapter_id = ?");
            $totalStmt->bind_param("i", $chapterId);
            $totalStmt->execute();
